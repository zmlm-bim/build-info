...
import { getProjecInfo, addProjecInfo, editProjecInfo, deleteProjecInfo } from "@/api/index";
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
      getProjecInfo(this.searchForm).then(res => {
        this.loading = false;
        if (res.success) {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.modalType === 0) {
            ...
            delete this.form.id;
            addProjecInfo(this.form).then(res => {
            ...
            // 编辑
            editProjecInfo(this.form).then(res => {
              this.submitLoading = false;
              if (res.success) {
                this.$Message.success("操作成功");
                this.getDataList();
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
          deleteProjecInfo({ids: v.id}).then(res => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.clearSelectAll();
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          // 批量删除
          deleteProjecInfo({ids: ids}).then(res => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.clearSelectAll();
