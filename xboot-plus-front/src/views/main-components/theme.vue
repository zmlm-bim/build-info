...
export default {
  ...
  methods: {
    ...
    changeAccordion(v) {
      this.$store.commit("setAccordion", v);
    },
    changeMenuWidth(v) {
    ...
    changeWatermark(v) {
      if (v) {
        ...
        if (type == 0) {
          config.watermark_txt = this.getUserInfo().username;
        } else if (type == 1) {
          config.watermark_txt = this.getUserInfo().mobile;
        } else {
      ...
      this.$store.commit("setWatermark", v);
    },
    changeWMAlpha(v) {
      this.$store.commit("setWMAlpha", v);
      this.changeWatermark(true);
    },
    changeWMFontsize(v) {
      this.$store.commit("setWMAFontsize", v);
      this.changeWatermark(true);
    },
    changeWeakMode(v) {
      if (v) {
