  ...
  <div class="search">
    ...
    <Modal :title="modalTitle" v-model="modalVisible" :mask-closable='false' :width="720">
      <Form ref="form" :model="form" :rules="formValidate"  label-position="top">
          ...
          <Row :gutter="32">
            <Col span="12">
              <FormItem label="报名数量" prop="bmsl" >
                <InputNumber v-model="form.bmsl" style="width:100%"></InputNumber>
              </FormItem>
            </Col>
            <Col span="12">
              <FormItem label="中标供应商" prop="zbgys" >
                <Input v-model="form.zbgys" clearable/>
              </FormItem>
            </Col>
          </Row>
          <Row :gutter="32">
            <Col span="12">
              <FormItem label="中标金额" prop="zbje" >
...
import { getBiddingInfoList, addBiddingInfo, editBiddingInfo, deleteBiddingInfo } from "@/api/index";
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
      getBiddingInfoList(this.searchForm).then(res => {
        this.loading = false;
        if (res.success) {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.modalType === 0) {
            ...
            // 编辑
            editBiddingInfo(this.form).then(res => {
              this.submitLoading = false;
              if (res.success) {
                this.$Message.success("操作成功");
                this.getDataList();
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
          deleteBiddingInfo({ids: v.id}).then(res => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.clearSelectAll();
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          // 批量删除
          deleteBiddingInfo({ids: ids}).then(res => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.clearSelectAll();
